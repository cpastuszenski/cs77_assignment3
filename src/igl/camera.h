...
inline ray3f camera_ray(Camera* camera, const vec2f& uv, const vec2f& auv) {
    ray3f rayl;
    if(not camera->orthographic) {
        auto q = vec3f((uv.x-0.5f)*camera->image_width*camera->focus_dist/camera->image_dist, (uv.y-0.5f)*camera->image_height*camera->focus_dist/camera->image_dist, -camera->focus_dist);
        auto e = vec3f(auv.x,auv.y,0) * camera->focus_aperture;
